...
void Slave::addOfferOperation(OfferOperation* operation)
{
  ...
  switch (operation->info().type()) {
    case Offer::Operation::CREATE_VOLUME: {
      resource = operation->info().create_volume().source();
      break;
    }
    case Offer::Operation::DESTROY_VOLUME: {
      resource = operation->info().destroy_volume().volume();
      break;
    }
    case Offer::Operation::CREATE_BLOCK: {
      resource = operation->info().create_block().source();
      break;
    }
    case Offer::Operation::DESTROY_BLOCK: {
      resource = operation->info().destroy_block().block();
      break;
    }
    case Offer::Operation::RESERVE:
    case Offer::Operation::UNRESERVE:
  ...
  usedResources[operation->framework_id()] += resource;
}
