...
* Prior to Mesos 1.0, environment variables prefixed by `SSL_` are used to control libprocess SSL support. However, we found that those environment variables may collide with some libraries or programs (e.g., openssl, curl). From Mesos 1.0, we deprecated those environment variables. Users should use the corresponding `LIBPROCESS_SSL_` version.
