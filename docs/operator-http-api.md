...
* `400 Bad Request`: If `info` is not well-formed.
...
* `500 Internal Server Error`: If anything goes wrong.
...
it from being launched again. The master and the agent will think the resource
provider has disconnected, similar to agent disconnection.
